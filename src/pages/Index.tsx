...
// Multi-idioma "safe" enquanto ainda NÃO tens rotas /en e /pt.
// Usa querystring (?lang=en|pt) para não gerar links quebrados.
function makeAlternatesHome() {
  return [
    { hreflang: "en", href: `${base}/?lang=en` },
    { hreflang: "pt", href: `${base}/?lang=pt` },
    { hreflang: "x-default", href: `${base}/?lang=en` },
  ];
}

const Index = () => {
  const { user, signOut } = useAuth();
  const { t, formatPrice, basePricesUSD, language } = useLanguage();
  ...
  const handleSubscribe = (plan: "standard" | "pro") => {
    ...
    if (user) setPaymentModalOpen(true);
  };
  ...
  const seo = isPT
    ? {
        title: "DocMind AI — Análise Inteligente de Documentos",
        description:
          "Transforme seus documentos com IA. Resumos, explicações simples, sugestões de melhoria e versões aprimoradas em segundos.",
        canonical: `${base}/?lang=pt`,
        lang: "pt-BR",
      }
    : {
        title: "DocMind AI — Smart Document Analysis",
        description:
          "Transform your documents with AI. Summaries, simple explanations, smart suggestions and improved versions in seconds.",
        canonical: `${base}/?lang=en`,
        lang: "en",
      };
            ...
            <div className="grid md:grid-cols-2 lg:grid-cols-3 gap-6 max-w-6xl mx-auto">
              {[
                ...
                },
                {
                  ...
                  desc: t("improvementSuggestionsDesc"),
                  color: "text-emerald-500",
                  bg: "bg-emerald-50",
                },
                {
                  icon: Wand2,
                  title: t("improvedVersionTitle"),
                  desc: t("improvedVersionDesc"),
                  color: "text-purple-500",
                  bg: "bg-purple-50",
                },
                {
                  icon: GraduationCap,
                  title: t("proPlanFeature3").replace("📚 ", ""),
                  desc:
                    t("proPlanFeature3").replace("📚 ", "") +
                ...
                <div
                  key={f.title}
                  className="bg-slate-50 rounded-2xl p-8 hover:shadow-lg transition-all hover:-translate-y-1"
                >
            ...
            <div className="grid md:grid-cols-3 gap-6 max-w-5xl mx-auto">
              ...
              <div className="bg-white rounded-3xl p-8 border border-slate-200">
                ...
                <Button
                  ...
                  asChild={!user}
                >
                  {user ? <span>{t("subscribeStandard")}</span> : <Link to="/auth">{t("subscribeStandard")}</Link>}
                </Button>
              </div>
              ...
              <div className="bg-white rounded-3xl p-8 ring-2 ring-indigo-500 shadow-xl relative">
                ...
                <Button
                  ...
                  asChild={!user}
                >
                  {user ? <span>{t("subscribePro")}</span> : <Link to="/auth">{t("subscribePro")}</Link>}
                </Button>
              </div>
            </div>
          </div>
